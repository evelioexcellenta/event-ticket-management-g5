...
function Cart(props) {
    ...
    return props.cartGlobal.cartList.map((val) => {
      return (
        <tr key={val.id}>
          ...
          <td className="align-middle">
            <button
              onClick={() => deleteCartHandler(val.id)}
  ...
  const renderSubtotalPrice = () => {
    ...
    for (let i = 0; i < props.cartGlobal.cartList.length; i++) {
      subtotal +=
        props.cartGlobal.cartList[i].price *
        props.cartGlobal.cartList[i].quantity
    }
    return subtotal
  }
    ...
    if (state.payment > renderTotalPrice()) {
      alert(
        `Sukses, Anda mendapatkan kembalian ${
          state.payment - renderTotalPrice()
        }`
      )
